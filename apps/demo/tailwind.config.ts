...
export default {
  ...
  // The prompt layer's (mock) generator emits classes at runtime, so Tailwind can't see
  // them in source. Safelist the families/values it can produce so their CSS always exists.
  safelist: [
    {
      pattern:
